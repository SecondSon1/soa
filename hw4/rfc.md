...
Компания развивает онлайн-банк с 10 млн пользователей. Транзакционные уведомления (подтверждение перевода, списание средств, вход в аккаунт) являются критичными - их недоставка напрямую влияет на пользовательский опыт и безопасность.
...
|-------|---------------------|------------|-----------|-------------|
| Push | 0.1–1 с | Средняя (~90–95% до устройства) | Бесплатно | Нет гарантии доставки до устройства; не работает без приложения и при выключенном телефоне |
...
ASR-3. Наблюдаемость процесса доставки
...
| FR4 (дедупликация при failover) |  Проще: единое состояние в PG |  Сложнее: eventual consistency, нужны LWT |
| FR5 (статус доставки) |  Один SQL-запрос |  Корреляция событий из разных топиков |
| NFR1 (latency <= 1 с p99) |  ~200–500 мс |  ~500–900 мс (близко к лимиту) |
| NFR2 (delivery rate >= 99.99%) |  ACID-гарантии PG |  Cassandra RF=3 + at-least-once |
| NFR3 (availability >= 99.99%) |  SPOF-риск оркестратора (митигируется репликами) |  Нет единой точки отказа |
| NFR5 (>= 1 000 RPS) |  PG: ~3K write/s - с запасом |  Cassandra: десятки тысяч write/s |
| NFR6 (observability) |  Отличная: единая точка |  Хорошая, но требует инфраструктуры корреляции |
